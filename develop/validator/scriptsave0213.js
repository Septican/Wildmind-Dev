...
function validation(send_trigger, formid) {
    ...
    // ellenőrző és validáló függvények
		...
		this.checkCountry = function() {
			...
			$('select[name="country[]"]').on('blur change', function() {
	...
	if ( send_trigger === true ) {
			...
			elements = document.getElementsByClassName('maindatas');
			 ...
			 if ( this.oName == "" || this.oEmail == "" || this.oPhone == "" || this.oAdrs == "" || this.oDate == "" || this.thepass == "" || this.confirmpass == "" || !this.validatePhone() || !this.validateEmail() || thepass != this.confirmpass || !this.validateDate() || !this.validateName() || !this.checkTerms() || !this.validateCountry() || !this.validateAddress() ) { 
